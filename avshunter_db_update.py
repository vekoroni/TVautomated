...
---------------------
1.  Patches the existing 397,337-row DB in-place (structure_quality fix)
2.  Fetches missing date ranges from Polygon to add ~580K new rows
...
GAPS CLOSED
-----------
  CRITICAL  Temporal coverage:  0.8yr → 3yr  (adds RISK_ON, EXPANSION, DISTRIBUTION)
...
---------------------------------------------
  ...
  2023-01-01 → 2023-12-31  RISK_ON recovery, MARKUP setups, VIX<15 sustained
  2024-01-01 → 2024-11-30  RISK_ON continuation, rate-cut rally, LATE maturity
  (2024-12 → 2025-03 already in existing DB — skipped via dedup)
...
-----------------------
  ...
  # Full run (4-6 hours at free tier):
  python avshunter_db_update.py

  # Custom paths:
  python avshunter_db_update.py ^
    --existing  C:\\path\\to\\actuarial_database.parquet ^
    --universe  C:\\path\\to\\clean_universe__with_sector.csv ^
    --output    C:\\path\\to\\actuarial_database_updated.parquet

GOOGLE COLAB
------------
  !pip install pyarrow pandas numpy requests
  from google.colab import files
  files.upload()   # upload actuarial_database.parquet + clean_universe*.csv
  !python avshunter_db_update.py
  files.download('actuarial_database_updated.parquet')

OUTPUT
------
  actuarial_database_updated.parquet   — drop-in replacement for existing DB
...
DEFAULT_OUTPUT      = r"C:\Users\ACKVerissimo\vanguard\data\staging\actuarial_database_v7_updated.parquet"
DEFAULT_LOG         = "actuarial_db_update_log.csv"

# Date ranges to fetch (inclusive). Existing DB covers ~mid-2024 to early-2025.
# Extra buffer of 60 days before each window for EMA/ATR lookback computation.
FETCH_WINDOWS = [
...
# Polygon rate limits
CALLS_PER_MINUTE  = 5     # free tier — change to 500 for Starter plan
...
def main():
    ...
    parser.add_argument("--existing",  default=DEFAULT_EXISTING_DB,
                        help="Path to existing actuarial_database.parquet")
    parser.add_argument("--universe",  default=DEFAULT_UNIVERSE,
                        help="Path to clean_universe CSV with ticker list")
    ...
    parser.add_argument("--rate",      type=int, default=None,
                        help="Polygon API calls/min (default: 5 free tier; 500 Starter plan)")
    ...
    new_rows, log_rows = fetch_new_rows(tickers, existing_keys, test_mode=args.test)
    ...
    print("\n" + "=" * 70)
    print("  DONE")
    print(f"  Deploy: copy {Path(args.output).name} → {existing_path}")
    print("  Then run: python morning_validation.py")
    print("=" * 70)
