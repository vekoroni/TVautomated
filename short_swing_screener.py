...
# Gate thresholds
# SPRINT 3: Phase D added with CRABEL_READY requirement. POST_EARNINGS_MOMENTUM bypass.
GATE_PHASES           = ['C', 'D', 'E']
GATE_CRABEL_REQUIRED_FOR_D = True      # Phase D only valid with Crabel compression
GATE_EARNINGS_BYPASS_FLAG  = 'POST_EARNINGS_MOMENTUM'
...
def gate_1_phase(df: pd.DataFrame, logger: logging.Logger) -> pd.DataFrame:
    """
    Gate 1: Phase C, D (with Crabel compression), or E.
    SPRINT 3: Phase D added — valid for short-swing when Crabel compression
    is confirmed. POST_EARNINGS_MOMENTUM bypasses phase gate entirely.
    """
    if 'phase' not in df.columns:
        logger.warning("Gate 1: 'phase' column missing — skipping")
        return df

    # POST_EARNINGS_MOMENTUM bypass
    earnings_bypass = pd.Series(False, index=df.index)
    for col in ('signal_type', 'catalyst_flag'):
        if col in df.columns:
            earnings_bypass = earnings_bypass | df[col].str.upper().str.contains(
                GATE_EARNINGS_BYPASS_FLAG, na=False
            )

    # Phase C and E: always pass
    phase_ce = df['phase'].isin(['C', 'E'])

    # Phase D: only with Crabel compression
    if GATE_CRABEL_REQUIRED_FOR_D and 'crabel_state' in df.columns:
        phase_d = df['phase'].isin(['D']) & df['crabel_state'].isin(GATE_CRABEL_STATES)
    else:
        phase_d = df['phase'].isin(['D'])

    passed = df[earnings_bypass | phase_ce | phase_d].copy()
    logger.info(
        f"Gate 1 (Phase C/D/E):   {len(df):>4} → {len(passed):>4} signals "
        f"[CE:{phase_ce.sum()} D:{phase_d.sum()} bypass:{earnings_bypass.sum()}]"
    )
    return passed
