...
# ---------------------------------------------------------------------------
# v2.1 — fixes from the 2026-08-09 17:52 and 17:59 live runs
#   1. NameError 'trend'      — the v2.0 B-02 patch block spanned the 5-day
#                               trend calculation and deleted it. Restored.
#   2. DGS30 NameError        — used `lookback_days`, out of scope in the curve
#                               function. Now an explicit 30.
#   3. Tuple not imported     — `-> Tuple[list, str]` annotation.
#   4. curve vs yield_curve   — build_composite's parameter is `yield_curve`;
#                               the B-05/B-06 additions used `curve`. Latent
#                               NameError on exactly the stale/negative-delta
#                               path those fixes exist to trigger.
#   5. DNS failure            — home.treasurydirect.gov does not resolve. That
#                               host came from a COMMENT in v1 and was never a
#                               live call; implementing it verbatim made a stale
#                               note into a runtime dependency. Now three
#                               endpoints tried in order. STILL UNVERIFIED from
#                               a networked machine.
#   6. Calendar wrote 0 rows  — the more serious one. get_week_of_month uses
#                               CALENDAR-week arithmetic, so in a month starting
#                               Fri/Sat/Sun the 2nd Tuesday lands in "week 3".
#                               August 2026 starts on a Saturday, so 11/12/13
#                               Aug all missed a schedule expecting week 2 and
#                               a week carrying $125bn of supply produced an
#                               empty calendar. Replaced with
#                               nth_weekday_of_month; verified across 24 months.
# ---------------------------------------------------------------------------
AUCTION_WINDOW_DAYS   = 5          # forward-looking, configurable
# The host in the original comment (home.treasurydirect.gov) does not resolve —
# the 17:59 run failed with getaddrinfo/NameResolutionError, which is DNS, not
# proxy or TLS. Try known endpoints in order; the Fiscal Data API is the modern
# documented one. NONE of these has been reached from this machine yet, so the
# list is deliberately plural and the fallback path is exercised on failure.
TREASURY_ENDPOINTS = [
    ("TREASURYDIRECT_WS",
     "https://www.treasurydirect.gov/TA_WS/securities/upcoming",
     {"format": "json"}),
    ("TREASURYDIRECT_WS_ALT",
...
TREASURY_UPCOMING_URL = TREASURY_ENDPOINTS[0][1]
TREASURY_TIMEOUT      = 20
...
def fetch_treasury_calendar(timeout: int = TREASURY_TIMEOUT) -> Tuple[list, str]:
    """
    Upcoming coupon auctions. Walks TREASURY_ENDPOINTS in order.

    Returns (rows, source). rows: [{"date": date, "tenor": "10Y",
    "type": str, "offering_usd_bn": float|None}]
    """
    for name, url, params in TREASURY_ENDPOINTS:
        try:
            r = requests.get(url, params=params, timeout=timeout,
                             headers={"User-Agent": "Mozilla/5.0 (AVSHUNTER bond-macro/2.1)"})
            if r.status_code != 200:
                log.warning(f"{name}: HTTP {r.status_code}")
                continue
            payload = r.json()
        except Exception as e:
            log.warning(f"{name} unreachable ({type(e).__name__}: {str(e)[:120]})")
            continue

        # TreasuryDirect returns a bare list; Fiscal Data wraps it in {"data": [...]}
        items = payload.get("data") if isinstance(payload, dict) else payload
        if not isinstance(items, list) or not items:
            log.warning(f"{name}: no rows in payload")
            continue

        rows = []
        for it in items:
            tenor = _normalise_tenor(it.get("securityTerm") or it.get("security_term"))
            if tenor is None:
                continue                      # bills and TIPS are not the supply risk we gate on
            raw = (it.get("auctionDate") or it.get("auction_date")
                   or it.get("issueDate") or it.get("issue_date"))
            if not raw:
                continue
            try:
                ...
                continue
            amt = (it.get("offeringAmount") or it.get("offering_amt")
                   or it.get("offering_amount"))
            try:
                amt = float(amt) / 1e9 if amt not in (None, "", "null") else None
            except (TypeError, ValueError):
                amt = None
            rows.append({"date": adate, "tenor": tenor,
                         "type": it.get("securityType") or it.get("security_type"),
                         "offering_usd_bn": amt})
        if rows:
            log.info(f"Auction calendar from {name}: {len(rows)} coupon auctions")
            return sorted(rows, key=lambda x: x["date"]), name

    log.warning("All TreasuryDirect endpoints failed — heuristic fallback")
...
def get_week_of_month(d: date) -> int:
    """Calendar week within the month (1 = first). Retained for compatibility."""
    first_weekday = d.replace(day=1).weekday()
    return (d.day + first_weekday - 1) // 7 + 1


def nth_weekday_of_month(d: date, weekday: int, n: int) -> bool:
    """
    Is `d` the Nth <weekday> of its month?

    The 17:59 run wrote a calendar with ZERO rows for a week carrying three
    auctions. Cause: get_week_of_month uses CALENDAR-week arithmetic, so when a
    month starts on Fri/Sat/Sun the 2nd Tuesday lands in "week 3". August 2026
    starts on a Saturday, so 11/12/13 Aug all evaluated to week 3 against a
    schedule expecting week 2, and every auction was missed.

    Auction schedules are defined as "the 2nd Wednesday", not "a Wednesday in
    calendar week 2". Counting occurrences is both correct and calendar-safe.
    """
        ...
        for offset in range(0, AUCTION_WINDOW_DAYS + 1):
            ...
            for a in AUCTION_SCHEDULE:
                ...
                if months != "all" and d.month not in months:
                    continue
                if nth_weekday_of_month(d, a["weekday"], a["week_of_month"]):
