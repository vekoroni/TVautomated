...
def _compute_priority_score(sig):
    ...
    # S1: Blend signal_trust_score into priority score
    # trust_score (0-100) contributes a ±10 point adjustment:
    #   trust=100 → +5 pts bonus
    #   trust=80  →  0 pts (neutral, this is the minimum for A-book)
    #   trust=60  → -5 pts mild penalty
    #   trust<40  → -10 pts strong penalty
    trust = float(sig.get("signal_trust_score") or sig.get("mv__signal_trust_score") or 80)
    trust_adj = (trust - 80) * 0.10   # ±10 pt range centred on trust=80
    raw = raw + trust_adj
...
# ── S1: Trust filter endpoint ─────────────────────────────────────────────────
@app.route("/api/trust_filter")
def api_trust_filter():
    """
    S1 endpoint: return all signals for a run filtered and sorted by
    signal_trust_score and data_quality_state.

    Query params:
      run_id        — specific run (default: latest)
      min_trust     — minimum signal_trust_score (default: 0 = show all)
      dq_state      — comma-separated states to include (default: FRESH,WARNING,STALE,BLOCKED)
      direction     — CALL, PUT, or blank for both
      sort          — trust_desc (default), priority_desc, rr_desc

    Returns JSON with:
      signals[]     — ranked signal objects with trust fields prominent
      summary       — counts by dq_state and trust band
    """
    try:
        runs = _list_runs()
        run_id = request.args.get("run_id", "").strip() or (runs[0] if runs else "")
        ...
        dq_states = {s.strip().upper() for s in
                     request.args.get("dq_state", "FRESH,WARNING,STALE,BLOCKED").split(",")}
        direction = request.args.get("direction", "").strip().upper()
        sort_by   = request.args.get("sort", "trust_desc").lower()

        def _trust(s):
            try: return int(float(s.get("signal_trust_score") or
                                   s.get("mv__signal_trust_score") or 0))
            except: return 0

        def _dq(s):
            return str(s.get("data_quality_state") or
                        s.get("mv__data_quality_state") or "UNKNOWN").upper()
        ...
        for s in signals:
            ...
            if d not in dq_states:  continue
            if direction and str(s.get("direction","") or "").upper() != direction:
                continue
            filtered.append(s)

        # Sort
        if sort_by == "priority_desc":
            filtered.sort(key=lambda x: float(x.get("priority_score",0) or 0), reverse=True)
        elif sort_by == "rr_desc":
            filtered.sort(key=lambda x: float(x.get("rr_options",0) or x.get("rr",0) or 0), reverse=True)
        else:  # trust_desc (default)
            filtered.sort(key=_trust, reverse=True)

        # Summary counts
        trust_bands = {"high_90_100": 0, "good_80_89": 0, "caution_60_79": 0, "low_below_60": 0}
        dq_counts   = {}
        for s in filtered:
            t = _trust(s)
            d = _dq(s)
            dq_counts[d] = dq_counts.get(d, 0) + 1
            if t >= 90:   trust_bands["high_90_100"] += 1
            elif t >= 80: trust_bands["good_80_89"]  += 1
            elif t >= 60: trust_bands["caution_60_79"] += 1
            else:         trust_bands["low_below_60"] += 1

        # Build lightweight response — include key fields + all S1 fields
        s1_fields = ["signal_trust_score","data_quality_state","stale_reason",
                     "rationale_string","bar_data_source","bar_data_days_old",
                     "bar_data_asof","macro_age_hours","actuarial_no_match",
                     "options_quote_asof","macro_generated_at"]
        core_fields = ["ticker","direction","structural_tier","mv_verdict",
                       "mv_score","priority_score","rr_options","rr",
                       "ev2_ev_conf_adj","options_score","dte","strike","expiry",
                       "wyckoff_phase","setup_type","options_verdict","mv_position_size"]
        all_fields = core_fields + s1_fields

        out_signals = []
        for s in filtered:
            row = {f: s.get(f, "") for f in all_fields}
            out_signals.append(row)

        return jsonify({
            "ok":       True,
            "run_id":   run_id,
            "total":    len(out_signals),
            "summary":  {"by_dq_state": dq_counts, "by_trust_band": trust_bands},
            "signals":  out_signals,
        })
    except Exception as e:
        import traceback
...
# ── End S1 trust filter endpoint ──────────────────────────────────────────────


# ─── ENTRY POINT ───────────────────────────────────────────────────────────────
if __name__ == "__main__":
    print("=" * 65)
    print("  AVSHUNTER · INTELLIGENCE LAB v2.1  (Sprint 1 — Trust & Observability)")
