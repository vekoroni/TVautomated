r"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  AVSHUNTER · INTELLIGENCE LAB v2.2                                         ║
║  Port: 5002                                                                 ║
║                                                                             ║
║  Changes v2.1:                                                              ║
║    LAB-01  LRU cache (max 5 runs) — prevents unbounded memory growth       ║
║    LAB-02  future_momentum_bucket / win_rate_10d_v2 surfaced to scorer     ║
║    LAB-03  EV normalisation updated for PASS_SMALL range (0.0002–0.0098)   ║
║    LAB-04  api_enter_trade: entry_state_hash reads state_v2_key from sig   ║
║    LAB-05  _find_superbrain_dir: logs when fallback fires                  ║
║    LAB-06  /api/regime_drift endpoint — cross-run momentum bucket detector ║
║  Changes v2.2:                                                              ║
║    D3   sb_final_verdict removed from enter_trade gate (advisory only)     ║
║    D4   CORS restricted to localhost; host bound to 127.0.0.1              ║
║    D5   export_csv uses union-field schema (first-row truncation fix)      ║
║    M2   Traceback sanitised from API responses — logged server-side only   ║
║    M3   run_id validated (YYYYMMDD_HHMMSS format) before file-system ops   ║
║    M4   Directional stop sanity: CALL inv < entry, PUT inv > entry         ║
...
# D4: Restrict CORS to localhost only. The Lab has write-side endpoints
# (/api/enter_trade) that create live trade contracts. Open CORS is unacceptable.
...
# LAB-01: LRU cache — bounded to 5 runs to prevent unbounded memory growth.
# OrderedDict preserves insertion order; we evict oldest (first) when full.
_run_cache: OrderedDict = OrderedDict()
_RUN_CACHE_MAX = 5


def _cache_get(run_id: str):
    if run_id in _run_cache:
        _run_cache.move_to_end(run_id)   # mark as most recently used
...
def _cache_put(run_id: str, payload: dict) -> None:
    ...
    while len(_run_cache) > _RUN_CACHE_MAX:
        evicted = next(iter(_run_cache))
        _run_cache.pop(evicted)
        print(f"  ♻ Cache evicted: {evicted} (LRU, max={_RUN_CACHE_MAX})")
...
def _valid_run_id(run_id: str) -> bool:
    """
    M3: Validate run_id format before it reaches file system operations.
    Valid formats: YYYYMMDD_HHMMSS (15 chars) or YYYYMMDD (8 chars).
    All chars must be digits after removing the single underscore separator.
    """
    rid = str(run_id or "").strip()
    if not rid:
        return False
    normalised = rid.replace("_", "")
    return normalised.isdigit() and len(rid) in (8, 15)
...
def _find_superbrain_dir(run_dir):
    ...
    if candidates:
        # LAB-05: log fallback so stale-folder issues are visible in server output
        print(f"  ⚠ _find_superbrain_dir: exact folder absent, using fallback → {candidates[0].name}")
        return candidates[0]
    return exact
...
def _compute_priority_score(sig):
    ...
    # LAB-02: actuarial v2 fields — future momentum bucket confidence
    fmb      = _s("future_momentum_bucket") or _s("vg__future_momentum_bucket")
    fmb_conf = _f("future_momentum_bucket_confidence") or _f("vg__future_momentum_bucket_confidence")
    wr_v2    = _f("win_rate_10d_v2") or _f("vg__win_rate_10d_v2")
    ...
    # LAB-03: EV normalisation updated for PASS_SMALL range observed in live runs.
    # Previous range assumption (0.003–0.625) was pre-fix. Live EV is 0.0002–0.0098.
    # New: scale so that 0.01 EV = 1.0 (full credit), anything above is capped.
    w_ev2 = min(ev_adj / 0.01, 1.0) * 0.10 if ev_adj > 0 else 0.0
    ...
    # LAB-02: future_momentum_bucket confidence modifier (±5pts max).
    # High-confidence aligned momentum boosts; misaligned or uncertain dampens.
    if fmb and fmb_conf >= 0.55:
        direction = _s("direction")
        bullish = fmb in ("STRONG_UP", "MODERATE_UP", "MOMENTUM_UP")
        bearish = fmb in ("STRONG_DOWN", "MODERATE_DOWN", "MOMENTUM_DOWN")
        aligned = (bullish and direction == "CALL") or (bearish and direction == "PUT")
        opposed = (bullish and direction == "PUT")  or (bearish and direction == "CALL")
        if aligned:
            raw += 5.0 * fmb_conf        # max +5pts at 100% confidence
        elif opposed:
            raw -= 5.0 * fmb_conf        # max -5pts penalty

    # LAB-02: actuarial win rate v2 scalar
    if wr_v2 and 0.0 < wr_v2 <= 1.0:
        wr_scalar = 1.0 + (wr_v2 - 0.50) * 0.20   # ±10% range: 35%→0.97x, 65%→1.03x
        raw *= wr_scalar
...
def _load_run(run_id, force_reload=False):
    ...
    for sig in result["signals"]:
        ...
        # LAB-02: expose v2 actuarial fields as bare keys for priority scorer and MVE
        sig.setdefault("future_momentum_bucket",             vg.get("future_momentum_bucket",""))
        sig.setdefault("future_momentum_bucket_confidence",  vg.get("future_momentum_bucket_confidence", 0.0))
        sig.setdefault("future_momentum_bucket_distribution",vg.get("future_momentum_bucket_distribution",""))
        sig.setdefault("win_rate_10d_v2",                    vg.get("win_rate_10d_v2", 0.0))
        sig.setdefault("win_rate_5d_v2",                     vg.get("win_rate_5d_v2",  0.0))
        sig.setdefault("win_rate_20d_v2",                    vg.get("win_rate_20d_v2", 0.0))
        sig.setdefault("state_v2_key",                       vg.get("state_v2_key",""))
...
def api_reload_morning():
    ...
    return jsonify({"ok": True, "message": msg, "cache_size": len(_run_cache)})
...
def api_enter_trade():
    ...
    try:
        ...
        # M3: validate run_id before file-system access
        if not _valid_run_id(run_id):
            return jsonify({"ok":False,"error":f"Invalid run_id format: {run_id}"}), 400
        ...
        sb_v      = sig.get("sb_final_verdict","").upper()   # advisory only — not a gate

        # D3: Sovereign production gate.
        # sb_final_verdict is display/audit context only — it must NOT authorise trade entry.
        # Allowing legacy verdicts to bypass the EIL/options/v5 architecture
        # is a ghost-field capital-risk bug.
        tradeable = (
        ...
        if not tradeable:
            return jsonify({
                "ok": False,
                "error": (
                    f"{ticker}: blocked by production gate — "
                    f"options_verdict={opt_v}, campaign_verdict={camp_v}, thesis_decision={thesis}. "
                    f"sb_final_verdict={sb_v} is advisory context only and cannot authorise execution."
                )
            }), 400
        ...
        # M4: directional stop sanity — catches operator entry errors before capital is deployed
        if direction == "CALL" and invalidation_price >= entry_price:
            return jsonify({
                "ok": False,
                "error": f"CALL trade: invalidation_price ({invalidation_price}) must be BELOW entry_price ({entry_price})"
            }), 400
        if direction == "PUT" and invalidation_price <= entry_price:
            return jsonify({
                "ok": False,
                "error": f"PUT trade: invalidation_price ({invalidation_price}) must be ABOVE entry_price ({entry_price})"
            }), 400
        ...
        # LAB-04: use actual state_v2_key from signal for outcome learning accuracy
        _state_hash = (
            sig.get("state_v2_key")
            or sig.get("vg__state_v2_key")
            or "eil_v2_unknown"
        )
        ...
        return jsonify({"ok": False, "error": "Internal server error — see server log"}), 500
...
def api_export_csv():
    try:
        ...
        buf = io.StringIO()
        # D5: union-field schema — prevents first-row truncation
        _fn: list[str] = []
        _fs: set[str] = set()
        for _r in filtered:
            for _k in _r.keys():
                if _k not in _fs:
                    _fs.add(_k)
                    _fn.append(_k)
        writer = csv.DictWriter(buf, fieldnames=_fn, extrasaction="ignore")
        writer.writeheader()
...
@app.route("/api/regime_drift")
def api_regime_drift():
    """
    LAB-06: Cross-run momentum bucket drift detector.
    Compares future_momentum_bucket distributions across the last N runs
    to detect regime rotation before it propagates into signal verdicts.

    Returns:
      drift_score  : 0.0 (stable) → 1.0 (high drift)
      verdict      : STABLE | MODERATE_DRIFT | HIGH_DRIFT
      snapshots    : per-run bucket distributions
    """
    try:
        n_runs = int(request.args.get("n", 5))
        runs   = _list_runs()[:n_runs]
        ...
        for r in runs:
            ...
            buckets = [
                str(s.get("future_momentum_bucket","")).strip()
                for s in sigs
                if str(s.get("future_momentum_bucket","")).strip()
                and str(s.get("future_momentum_bucket","")).strip() not in ("","None","nan")
            ]
            if not buckets:
                continue
            ...
            snapshots.append({
                "run_id":       r,
                "distribution": {k: round(v / total, 3) for k, v in dist.most_common()},
        ...
        if len(snapshots) < 2:
            return jsonify({
                ...
                "message":     f"Need ≥2 runs with future_momentum_bucket data. Found {len(snapshots)}.",
                "snapshots":   snapshots,
            })

        # Drift score: fraction of prior runs where dominant bucket differs from latest
        latest_dom = snapshots[0].get("dominant")
        prior_doms = [s.get("dominant") for s in snapshots[1:]]
        drift_score = sum(1 for d in prior_doms if d != latest_dom) / len(prior_doms)

        # Distribution entropy comparison (latest vs average prior)
        def _entropy(dist: dict) -> float:
            import math
            return -sum(p * math.log2(p) for p in dist.values() if p > 0)

        latest_entropy = _entropy(snapshots[0]["distribution"])
        prior_entropies = [_entropy(s["distribution"]) for s in snapshots[1:]]
        avg_prior_entropy = sum(prior_entropies) / len(prior_entropies) if prior_entropies else 0
        entropy_delta = latest_entropy - avg_prior_entropy

        verdict = (
            "HIGH_DRIFT"     if drift_score >= 0.67 else
            "MODERATE_DRIFT" if drift_score >= 0.33 else
            "STABLE"
        )

        # Actionable note for operator
        action_note = {
            "HIGH_DRIFT":     "Regime rotating — reduce position sizes, tighten EXECUTE threshold, re-run macro synthesis.",
            "MODERATE_DRIFT": "Momentum bucket shifting — monitor next run. Consider reducing 11-20D exposure.",
            "STABLE":         "Regime stable — current signal weights appropriate.",
        }.get(verdict, "")
        ...
        return jsonify({
            ...
            "latest_dominant_bucket": latest_dom,
            "latest_entropy":         round(latest_entropy, 3),
            "prior_avg_entropy":      round(avg_prior_entropy, 3),
            "entropy_delta":          round(entropy_delta, 3),
            "runs_compared":          len(snapshots),
            "snapshots":              snapshots,
        })
...
if __name__ == "__main__":
    print("=" * 65)
    print("  AVSHUNTER · INTELLIGENCE LAB v2.2")
    print("  EIL-primary | LRU cache | Regime drift detector")
    print("=" * 65)
    ...
    print("=" * 65)
    print("  Server       : http://127.0.0.1:5002  (localhost only)")
    print("=" * 65)
    # D4: bind to 127.0.0.1 — Lab has write-side trade entry endpoints.
