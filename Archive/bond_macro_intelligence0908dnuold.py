...
def fetch_treasury_calendar(timeout: int = TREASURY_TIMEOUT) -> Tuple[list, str]:
    ...
    try:
        ...
        log.warning(f"TreasuryDirect unreachable ({type(e).__name__}) — heuristic fallback")
        return [], "HEURISTIC_FALLBACK"
...
def get_yield_curve_state(verbose: bool = False) -> dict:
    ...
    try:
        s_30y = fetch_fred_series("DGS30", lookback_days=lookback_days)
...
def build_composite(auction: dict, yield_curve: dict,
    ...
    if _curve_delta < 0 and not any("curve" in w.lower() for w in warnings):
        warnings.append(f"Curve {curve_state} — score {_curve_delta:+d}: "
                        f"{curve.get('regime_implication', '')}".strip())
    if curve.get("russell_tailwind") is False:
    ...
    if curve.get("stale_flag"):
        score -= 10
        warnings.append(f"Yield curve data STALE — as_of {curve.get('as_of_date')} "
                        f"({curve.get('staleness_sessions')} sessions)")
