...
class ContractErrorCode:
    ...
    DAILY_DF_LIMITED_HISTORY = "DAILY_DF_LIMITED_HISTORY"
...
HARD_MIN_DAILY_ROWS = 260       # ~1y trading days; minimum for EMA200 stability + buffer
SOFT_MIN_DAILY_ROWS = 900       # ~3.6y; preferred for long-cycle context + actuarial density
BEST_MIN_DAILY_ROWS = 1250      # ~5y; full climate context

def _history_tier(n_rows: int) -> str:
    """Classify history depth for capability-gating."""
    if n_rows < HARD_MIN_DAILY_ROWS:
        return "INSUFFICIENT"
    if n_rows < SOFT_MIN_DAILY_ROWS:
        return "LIMITED_CONTEXT"
    if n_rows < BEST_MIN_DAILY_ROWS:
        return "FULL_CONTEXT"
    return "FULL_CLIMATE"
...
def validate_daily_df_integrity(
    daily_df: pd.DataFrame,
    soft_min_rows: int = SOFT_MIN_DAILY_ROWS,
    hard_min_rows: int = HARD_MIN_DAILY_ROWS,
    best_min_rows: int = BEST_MIN_DAILY_ROWS,
    ema_window: int = 200,
    ...
    # Length sufficiency (capability-gated)
    n_rows = int(len(df))
    tier = _history_tier(n_rows)

    # Hard fail: insufficient bars for stable EMA200 + basic regime metrics
    if n_rows < hard_min_rows:
        issues.append(ValidationIssue(
            code=ContractErrorCode.DAILY_DF_TOO_SHORT,
            message="daily_df insufficient history for stable long-rail indicators (EMA200) and regime classification.",
            field="daily_df",
            details={"rows": n_rows, "hard_min_rows": hard_min_rows, "soft_min_rows": soft_min_rows, "tier": tier}
        ))
        score -= 60

    # Soft degrade: still valid for 1–20 day trading logic, but long-cycle features are disabled
    elif n_rows < soft_min_rows:
        disabled = [
            "actuarial_long_cycle_matching",
            "multi_cycle_trend_maturity",
            "long_rail_climate_context",
        ]
        issues.append(ValidationIssue(
            code=ContractErrorCode.DAILY_DF_LIMITED_HISTORY,
            message="daily_df has limited history; trade logic can run, but long-cycle features are disabled and confidence is capped.",
            field="daily_df",
            details={
                "rows": n_rows,
                "hard_min_rows": hard_min_rows,
                "soft_min_rows": soft_min_rows,
                "best_min_rows": best_min_rows,
                "tier": tier,
...
def validate_vanguard_input(vin: VanguardInput) -> ValidationResult:
    ...
    # Determine history tier for score thresholding (prevents pipeline collapse on low-data tickers)
    try:
        n_rows = int(len(vin.daily_df)) if isinstance(vin.daily_df, pd.DataFrame) else 0
    except Exception:
        n_rows = 0
    tier = _history_tier(n_rows)

    # Tiered minimum score: stay conservative, but do not hard-fail purely on <900 bars.
    min_score_by_tier = {
        "INSUFFICIENT": 90,      # will also carry DAILY_DF_TOO_SHORT fatal issue
        "LIMITED_CONTEXT": 85,   # 1–20d logic valid; long-cycle features disabled
        "FULL_CONTEXT": 90,      # standard strict mode
        "FULL_CLIMATE": 90,      # preferred baseline; same threshold, but richer context
    }
    min_required = min_score_by_tier.get(tier, 90)

    # Non-fatal issues: informational warnings that should not block execution.
    nonfatal_codes = {ContractErrorCode.DAILY_DF_LIMITED_HISTORY}
    fatal_issues = [i for i in issues if i.code not in nonfatal_codes]

    ok = (len(fatal_issues) == 0) and (score >= min_required)
