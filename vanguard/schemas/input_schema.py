...
class TechnicalData:
    ...
    # AVSHUNTER Discovery enrichment — Crabel compression + macro regime
    # FIX: main.py checks vanguard_input.compression_ratio and vanguard_input.macro_regime
    # via hasattr(). These fields must exist on TechnicalData (populated by adapter)
    # AND be promoted onto VanguardInput so the hasattr() checks resolve True.
    compression_ratio: Optional[float] = None   # sourced from crabel_compression
    macro_regime: Optional[str] = None          # sourced from discovery macro_regime / active_regime
...
class VanguardInput:
    ...
    # hasattr(vanguard_input, 'compression_ratio') DIRECTLY on this object.
    # Adapter promotes these from technical after _build_technical_data().
