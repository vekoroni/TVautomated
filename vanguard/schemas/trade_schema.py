...
class EdgeAssessment:
    ...
    # ── SPRINT 2: Signal classification propagated from ActuarialOutcomes ────
    # CONTINUATION | TRANSITION | NO_EDGE
    # Declared here so position_sizing_engine, EIL, and Intelligence Lab
    # can consume signal_type from EdgeAssessment without importing outcomes.
    signal_type: str = "NO_EDGE"

    # Forward momentum confidence passed through from ActuarialOutcomes
    forward_momentum_confidence: float = 0.0

    # v6 bucket-aware edge quality (Fix 6 — EdgeDetector._bucket_edge_quality).
    # EARLY_EXPANSION_STRONG | BUCKET_STRONG | EARLY_CANDIDATE_MODERATE | LEGACY_MODERATE | WEAK
    # Replaces the legacy STRONG/MODERATE/WEAK from main.py for bucket-informed setups.
...
class TradePlan:
    ...
    # Execution gates — required confirmations before committing capital
    # Built by TradeBuilder._build_execution_gates()
    # Structure: {required: [...], preferred: [...], blocking: [...], summary: str}
    execution_gates: Optional[Dict] = None
