...
def test_governed_shadow_phase_runs_end_to_end_without_authority(tmp_path) -> None:
    ...
    assert status["health"] == "SHADOW_COMPLETE"
    ...
    assert status["ev_functional_health"] == "SHADOW_COMPLETE"
    assert status["morning_capital_permission"] == "NOT_EVALUATED_BY_SHADOW"
