...
AVSHUNTER_OUTPUT_DIRS = [
    ...
    BASE_USER / "dropbox" / "macro",   # macro_intelligence_latest + enrichment_delta
]
...
def on_pipeline_complete(output_csv_path: str, output_dir: Optional[str] = None) -> List[str]:
    synced = []
    ...
    if output_dir:
        result = sync_to_ma_inputs(source_dir=output_dir, max_age_hours=1, verbose=True, recursive=True)
        total = sum(result[k] for k in ["pipeline_output", "options_data", "chart", "screenshot", "raw_input"])
        if total > 0:
            print(f"  [MA_Inputs] Auto-synced {total} additional file(s) from {Path(output_dir).name}/")
