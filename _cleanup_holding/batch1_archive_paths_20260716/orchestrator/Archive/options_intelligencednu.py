...
try:
    ...
    print("✅ Crabel Precor state machine loaded")
except ImportError:
    CRABEL_PRECOR_AVAILABLE = False
    print("⚠️  wyckoff_crabel_precor_logic_v2 not found - running without state machine")
...
try:
    ...
    print("⚠️  macro_loader not found - falling back to PDF parsing")
...
class OptionsIntelligence:
    ...
    def _validate_macro_regime(self, macro_data):
        ...
        if risk_switch == 'ON' and risk_on_prob >= 0.70 and liquidity == 'FLOODED':
            ...
            print(f"  [Macro] 🔴 RED (Hostile — PUT/SELL_SETUP aligned, CALL reduced)")
    ...
    def _scan_tier(self, tickers, tier_num, wyckoff_threshold, win_prob_threshold, size_mult):
        ...
        for ticker in tickers:
            try:
                ...
                print(f"  [T{tier_num}] ✅ {ticker} {signal['setup_quality']}{precor_tag} (Win: {win_prob:.0%})")
