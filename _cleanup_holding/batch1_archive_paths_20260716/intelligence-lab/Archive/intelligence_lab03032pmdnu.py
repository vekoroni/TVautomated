...
def _load_run(run_id: str) -> dict:
    ...
    # ── 2. Options intelligence CSV (FIX-04: capability flag — options are optional)
    ...
    options_enabled = opt_path is not None
    opt_rows = _read_csv(opt_path) if options_enabled else []
    # Index by ticker for fast merge
    opt_map = {r.get("ticker", "").upper(): r for r in opt_rows}
    result["capabilities"] = {"options_enabled": options_enabled}
    ...
    # ── FIX-05: Schema validation — attach sb_schema_ok / sb_schema_issues to each signal
    REQUIRED_FIELDS = ['sb_final_verdict', 'sb_campaign', 'sb_conv_score', 'sb_current_stage']
    VALID_VERDICTS  = {'EXECUTE', 'ARMED', 'STAND_DOWN', 'WATCHLIST', ''}
    for sig in signals:
        issues = []
        for f in REQUIRED_FIELDS:
            if not sig.get(f):
                issues.append(f'missing:{f}')
        verdict = str(sig.get('sb_final_verdict', '')).strip().upper()
        if verdict and verdict not in VALID_VERDICTS:
            issues.append(f'invalid_verdict:{verdict}')
        conv  = sig.get('sb_conv_score')
        c_max = sig.get('sb_conv_score_max', 8)
        try:
            if conv is not None and float(conv) > float(c_max):
                issues.append(f'conv_score_overflow:{conv}>{c_max}')
        except (ValueError, TypeError):
            pass
        sig['sb_schema_ok']     = 'Y' if not issues else 'N'
        sig['sb_schema_issues'] = '; '.join(issues) if issues else ''
    ...
    result["stats"] = {
        ...
        # FIX-04: capability flags passed to UI
        "options_enabled"    : options_enabled,
        # FIX-06: expose nested summary fields for UI stats consistency
        "signal_counts"      : summary.get("signal_counts", {}),
        "schema_issues_count": sum(1 for s in signals if s.get("sb_schema_ok") == "N"),
    }
