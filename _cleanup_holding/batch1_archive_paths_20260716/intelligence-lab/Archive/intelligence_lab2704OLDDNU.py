...
def _glob_first(folder: Path, pattern: str) -> Path | None:
    """Return last (latest) match for a glob pattern inside folder, or None."""
    matches = sorted(folder.glob(pattern)) if folder.exists() else []
    return matches[-1] if matches else None  # latest if multiple


def _find_superbrain_dir(run_dir: Path) -> Path:
    """
    Return the superbrain output subfolder for a run, regardless of naming.

    The pipeline writes superbrain outputs to one of:
      run_dir / superbrain/               (standard evening run)
      run_dir / superbrain_TEST3/         (TEST3 named run)
      run_dir / superbrain_<any_suffix>/  (any named variant)

    Strategy:
      1. Prefer exact 'superbrain' folder if it exists and has CSVs.
      2. Otherwise find the latest-modified superbrain* subfolder that
         contains at least one superbrain_enriched_*.csv file.
      3. Fall back to 'superbrain' (may not exist — callers handle None).
    """
    ...
    # Scan all superbrain* subfolders, pick the one with enriched CSV
    candidates = sorted(
    ...
    # Nothing found — return the exact path; callers handle missing folder
    return exact
...
def _load_run(run_id: str, force_reload: bool = False) -> dict:
    ...
    # ── Discover superbrain output folder (handles superbrain_TEST3/ etc.)
    sb_dir = _find_superbrain_dir(run_dir)

    # ── 1. SuperBrain enriched CSV (all signals)
    # Try exact run_id match first, then any prefix (handles TEST3_ prefix)
    ...
    sum_path = _glob_first(sb_dir, f"superbrain_summary_{run_id}.json")
    if not sum_path:
        sum_path = _glob_first(sb_dir, f"superbrain_summary_*{run_id}.json")
    if not sum_path:
    ...
    wbs_sum_path = _glob_first(sb_dir, f"wall_break_summary_{run_id}.json")
    if not wbs_sum_path:
        wbs_sum_path = _glob_first(sb_dir, f"wall_break_summary_*{run_id}.json")
    if not wbs_sum_path:
    ...
    # morning_validation.py writes to: runs/{run_id}/morning_validation/morning_validated_trades_{run_id}.csv
    ...
    # Fallback: old location in superbrain dir
    if not mv_path:
        mv_path = _glob_first(sb_dir, f"morning_validation_{run_id}.csv")
    if not mv_path:
    ...
    for sig in result["signals"]:
        ...
        for k, v in eil.items():
            ...
            if field_key not in sig:
                sig[field_key] = v
        # Prefix mv__ fields from morning validation (live prices + drift + contract quotes)
        ...
        # tce_ fields have no mv_ prefix so they pass through as mv__tce_trigger_state etc.
        mv = mv_map.get(t, {})
        for k, v in mv.items():
            if k == "ticker":
                continue
        ...
        # ── FIX-IVP: after morning run, live IV is in mv__live_iv.
        # Overwrite ivp_label and iv_rank with real data so frontend renders correctly.
        _live_iv = mv.get("live_iv") or mv.get("mv_live_iv")
        if _live_iv and str(_live_iv) not in ("", "None", "nan", "0", "0.0"):
            try:
                _iv_f = float(_live_iv)
                # iv_rank is expressed as percentile 0-100; live_iv is decimal (e.g. 0.17)
                # Convert to percentage for display
                _iv_pct = _iv_f * 100 if _iv_f < 1 else _iv_f
                sig["opt__iv_rank"]     = round(_iv_pct, 1)
                sig["opt__ivp_label"]   = ("CHEAP" if _iv_pct < 20 else
                                           "FAIR"  if _iv_pct < 50 else "RICH")
                sig["iv_rank"]          = round(_iv_pct, 1)
                sig["ivp_label"]        = sig["opt__ivp_label"]
                sig["mv__iv_pct"]       = round(_iv_pct, 1)
                sig["mv__ivp_label"]    = sig["opt__ivp_label"]
            except (TypeError, ValueError):
                pass

        # ── FIX-TCE: expose TCE fields at bare keys so frontend reads them directly.
        # morning_validation writes: tce_trigger_state, tce_trigger_score,
        # tce_entry_type, tce_trigger_reason, tce_summary_line
        # After mv__ merge these become mv__tce_trigger_state etc.
        # Alias them to bare names so existing and future frontend code finds them.
        _tce_map = {
            ...
            "tce_summary_line":   ("mv__tce_summary_line",   "mv_tce_summary_line"),
            "mv_score":           ("mv__score",              "mv_score"),
            "mv_verdict":         ("mv__verdict",            "mv_verdict"),
            "mv_position_size":   ("mv__position_size",      "mv_position_size"),
            "live_price":         ("mv__live_price",         "mv_live_price"),
            "live_vwap":          ("mv__live_vwap",          "mv_live_vwap"),
        }
        for bare_key, (prefixed_key, raw_key) in _tce_map.items():
            if bare_key not in sig or not sig.get(bare_key):
                val = sig.get(prefixed_key) or mv.get(raw_key) or mv.get(bare_key)
...
def api_enter_trade():
    ...
    try:
        ...
        run_dir = RUNS_DIR / run_id
        sb_dir_et = _find_superbrain_dir(run_dir)
        sb_path = _glob_first(sb_dir_et, f"superbrain_enriched_{run_id}.csv")
        if not sb_path:
            sb_path = _glob_first(sb_dir_et, f"superbrain_enriched_*{run_id}.csv")
        if not sb_path:
            sb_path = _glob_first(sb_dir_et, "superbrain_enriched_*.csv")
        if not sb_path:
...
def api_export_csv():
    """
    Export filtered superbrain signals as a downloadable CSV.

    Query params:
      run_id   – specific run to export (default: latest)
      verdict  – comma-separated list of verdicts to include, e.g. EXECUTE,EXECUTE_WITH_RISK,ARMED
                 (default: all actionable verdicts)
      rr_min   – minimum R:R (default: no filter)
      ev_min   – minimum EV (default: no filter)
      camp     – campaign filter e.g. CORE_CAMPAIGN (default: all)
    """
    try:
        # ── Resolve run
        runs = _list_runs()
        ...
        # ── Apply filters
        verdicts = [v.strip().upper() for v in request.args.get("verdict", "EXECUTE,EXECUTE_WITH_RISK,ARMED").split(",") if v.strip()]
        rr_min   = request.args.get("rr_min", None)
        ev_min   = request.args.get("ev_min", None)
        camp     = request.args.get("camp", "").strip().upper()

        filtered = []
        for s in signals:
            v = (s.get("sb_final_verdict") or "").strip().upper()
            if verdicts and v not in verdicts:
                continue
            if rr_min is not None:
                try:
                    if float(s.get("rr", 0) or 0) < float(rr_min):
                        continue
            ...
            if ev_min is not None:
                try:
                    ev_val = float(
                        s.get("ev2_ev_conf_adj") or s.get("fd_ev_used") or
                        s.get("ev_final") or s.get("ev") or 0
                    )
                    if ev_val < float(ev_min):
                        continue
                except (ValueError, TypeError):
                    pass
            if camp and (s.get("sb_campaign") or "").strip().upper() != camp:
                continue
            filtered.append(s)

        if not filtered:
            return jsonify({"ok": False, "error": "No signals match the specified filters"}), 404
        ...
        filename = f"avshunter_signals_{run_id}.csv"
        return Response(
            csv_content,
            mimetype="text/csv",
...
if __name__ == "__main__":
    ...
    if runs:
        ...
        # Show which superbrain subfolder will be used
        from pathlib import Path as _P
        _sb = _find_superbrain_dir(RUNS_DIR / runs[0])
        print(f"  SB folder    : {_sb.name}  ({'exists' if _sb.exists() else 'MISSING'})")
        _enriched = list(_sb.glob("superbrain_enriched_*.csv")) if _sb.exists() else []
        print(f"  Enriched CSV : {_enriched[0].name if _enriched else 'NOT FOUND'}")
    else:
