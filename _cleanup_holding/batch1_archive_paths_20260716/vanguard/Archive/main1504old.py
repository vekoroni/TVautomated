...
class VanguardEngine:
    ...
    def analyze(self, vanguard_input: VanguardInput) -> VanguardSignal:
        ...
        # =========================
        # FIX: Mark the state as positional strategy (hold 1–20d, long calls/puts).
        # This tells edge_detector that intraday_rows=0 is expected — not a data failure —
        # so it does not penalise auction confidence or override the auction state.
        # StateVector has intraday_rows as an int field; positional_strategy is added here
        # as a lightweight annotation without modifying the dataclass schema.
        if outcomes is not None:
            try:
                object.__setattr__(state, 'positional_strategy', True)
            except (AttributeError, TypeError):
                state.__dict__['positional_strategy'] = True  # fallback for frozen dataclasses
        ...
        # Detect SETUP_FORMING: has_edge=False but rationale contains SETUP FORM
        # and EV is above the setup floor (meaningful positive EV, just below trade floor).
        # For a 1–20d positional strategy these are the highest-priority watchlist names.
        is_setup_forming = (
            not edge.has_edge
            and "SETUP FORM" in (edge.rationale or "")
            and float(getattr(edge, 'edge_magnitude', 0) or 0) > 0.005
        )
        ...
        elif is_setup_forming and discovery_flag:
            # EV positive but below regime trade floor. Surface as WATCHLIST so
            # the signal reaches superbrain for monitoring without full execution.
            # FIX: previously these fell silently to OBSERVE — they are now visible.
            verdict = "WATCHLIST"
            direction = getattr(edge, 'edge_direction', 'NONE')
            recommendation = f"SETUP_FORMING_{direction}" if direction and direction != 'NONE' else "SETUP_FORMING"

        # WEAK edge / negative EV falls through to OBSERVE
        ...
        return VanguardSignal(
            ...
            layer_2_result={
                ...
                "is_setup_forming": is_setup_forming,  # True = positive EV but below trade floor
