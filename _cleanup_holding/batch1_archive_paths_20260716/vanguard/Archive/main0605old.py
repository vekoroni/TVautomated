...
class VanguardEngine:
    ...
    def analyze(self, vanguard_input: VanguardInput) -> VanguardSignal:
        ...
        sample_confidence_bucket = _sample_confidence_bucket(
            outcomes.n_observations if outcomes else 0
        )
        ...
        elif edge.has_edge and edge_quality == "STRONG":
            verdict        = "ACTUARIAL_SUPPORT"
            recommendation = f"{edge.edge_direction}_EDGE_STRONG_{preferred_horizon}"

        elif edge.has_edge and edge_quality == "MODERATE":
            verdict        = "ACTUARIAL_MODERATE"
            recommendation = f"{edge.edge_direction}_EDGE_MODERATE_{preferred_horizon}"
