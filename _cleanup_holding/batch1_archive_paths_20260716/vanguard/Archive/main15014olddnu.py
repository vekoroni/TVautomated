    ...
    def analyze(self, vanguard_input: VanguardInput) -> VanguardSignal:
        ...
        return VanguardSignal(
            ...
            layer_2_result={
                ...
                # run_vanguard_from_packages._ov() can find them as top-level l2 keys.
                # Previously only win_rate_20d was present here; EVEngineV2 fell back to
                # a=0.40 (flat prior) for every row because all hit rates appeared zero.
                ...
                # 5-day horizon (FIX-04: was missing — causes DataQualityWarning in EVEngineV2)
                ...
                # 10-day horizon (FIX-04: was missing — causes DataQualityWarning in EVEngineV2)
                ...
                # Nested outcomes object for run_vanguard_from_packages._ov() fallback path
                # (FIX-04: _ov() checks outcomes_sub = l2.get("outcomes") first)
                "outcomes":                outcomes,
