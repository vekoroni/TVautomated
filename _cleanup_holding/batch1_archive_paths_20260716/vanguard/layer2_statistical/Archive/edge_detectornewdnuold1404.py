...
# =============================================================================
#  FIX-03: MODULE-LEVEL WRAPPER FOR RUNNER INTEGRATION
# =============================================================================
#
#  execution_intelligence_runner imports `enrich_vanguard_inputs` at module
#  level. Previously that function did not exist — EdgeDetector was a class
#  only, and the ImportError fallback silently ran a 2-line stub for every row.
#
#  This wrapper:
#    1. Builds a minimal ActuarialOutcomes from fields already in the row
#       (win_rate_*, expected_move_*, survival_prob, etc.).  When the full
#       Vanguard actuarial pipeline has run (FIX-04), these fields carry real
#       data.  Until then the function degrades gracefully to conservative
#       defaults — never raises.
#    2. Writes survival_prob, gamma_obstruction, path_cleanliness, and all
#       win_rate_* / expected_move_* keys to the row so EVEngineV2 sees them
#       under their canonical column names.
#    3. Preserves any values already set by upstream (setdefault semantics for
#       derived fields; hard-overwrite for actuarial fields that come from row).
#
#  The function is intentionally self-contained — no StateVector construction,
#  no full ActuarialQueryEngine call — because the runner does not have access
#  to the Vanguard DB at execution time.  Full actuarial enrichment happens in
#  the Vanguard pipeline (FIX-04); this wrapper surfaces whatever made it
#  through to the superbrain_enriched CSV.

def enrich_vanguard_inputs(row: dict) -> dict:
    """
    Module-level wrapper called by execution_intelligence_runner (FIX-03).

    Surfaces actuarial and structural fields from the row into the canonical
    keys that EVEngineV2 ev_inputs_from_row() reads.

    Fields written (only if not already a non-zero value):
        win_rate_5d / win_rate_10d / win_rate_20d  — from layer2__ or direct
        survival_prob                               — from prob_down proxy
        gamma_obstruction                           — from control_score proxy
        path_cleanliness                            — from data_quality proxy
    ...
    try:
        # ── Win rates: prefer direct 0-1 columns, fallback to layer2__ ×100 ──
        def _wr(direct: str, l2: str) -> float:
            v_direct = _f(direct)
            if 0 < v_direct <= 1.0:
                return v_direct
            v_l2 = _f(l2)
            if v_l2 > 0:
                return min(v_l2 / 100.0, 1.0)   # layer2__ is on 0-100 scale
            return 0.0

        wr5  = _wr("win_rate_5d",  "layer2__win_rate_5d")
        wr10 = _wr("win_rate_10d", "layer2__win_rate_10d")
        wr20 = _wr("win_rate_20d", "layer2__win_rate_20d")

        # Write win rates — these feed directly into EVEngineV2 _blend_win_probability
        if wr5  > 0: row["win_rate_5d"]  = wr5
        if wr10 > 0: row["win_rate_10d"] = wr10
        if wr20 > 0: row["win_rate_20d"] = wr20

        # ── Expected moves: prefer l3_ GARCH fields (post FIX-05), else layer2__ ──
        def _em(garch_key: str, l2_key: str, fallback_key: str) -> float:
            v = _f(garch_key) or _f(l2_key) or _f(fallback_key)
            return max(0.0, v)

        em5  = _em("l3_expected_move_1_5d",  "layer2__median_gain_if_up_5d",  "expected_move_5d")
        em10 = _em("l3_expected_move_6_10d", "layer2__median_gain_if_up_10d", "expected_move_10d")
        em20 = _em("l3_expected_move_11_20d","layer2__median_gain_if_up",     "expected_move_20d")

        if em5  > 0: row["expected_move_5d"]  = em5
        if em10 > 0: row["expected_move_10d"] = em10
        if em20 > 0: row["expected_move_20d"] = em20

        # ── IV tailwind from GARCH (l3_iv_tailwind_score) ────────────────────
        iv_tail = _f("l3_iv_tailwind_score") or _f("iv_tailwind_score")
        if iv_tail != 0.0:
            row["iv_tailwind_score"] = iv_tail

        # ── Survival probability: prefer direct field, derive from proxy ──────
        sp = _f("survival_prob")
        if sp <= 0.0 or sp > 1.0:
            # Proxy: 1 - prob_down_5pct (from layer2 if available)
            prob_down = _f("layer2__prob_down_5pct_before_up_10pct")
            if 0 < prob_down < 1:
                sp = max(0.0, 1.0 - prob_down)
            else:
                # Last resort: blend from value_acceptance_score (range 0-1)
                vas = _f("value_acceptance_score", 0.5)
                sp  = 0.40 + vas * 0.40           # 0.40–0.80 range
        row["survival_prob"] = min(1.0, max(0.0, sp))

        # ── Gamma obstruction: invert control_score (buyer/seller dominance) ──
        go = _f("gamma_obstruction")
        if go <= 0.0 or go > 1.0:
            cs = _f("control_score", 0.5)
            go = max(0.0, 1.0 - cs)
        row["gamma_obstruction"] = min(1.0, go)

        # ── Path cleanliness: data quality as a proxy for signal clarity ──────
        pc = _f("path_cleanliness")
        if pc <= 0.0 or pc > 1.0:
            dq = _f("data_quality_score", 60.0)
            pc = min(1.0, dq / 100.0)
        row["path_cleanliness"] = min(1.0, max(0.0, pc))

        # ── Data quality score: propagate if already set ──────────────────────
        if _f("data_quality_score") <= 0:
            # Derive from confidence_level if layer2 carried it
            cl = _f("layer2__confidence_level")
            if 0 < cl <= 1.0:
                row["data_quality_score"] = min(100.0, cl * 100.0)
            elif cl > 1.0:
                row["data_quality_score"] = min(100.0, cl)   # already 0-100 scale

    except Exception as _e:
        # Never raise — set conservative defaults and continue
        import logging
        logging.getLogger("edge_detector").warning(
            f"enrich_vanguard_inputs failed for {row.get('ticker','?')}: {_e}"
        )
        row.setdefault("survival_prob",     0.50)
        row.setdefault("gamma_obstruction", 0.20)
        row.setdefault("path_cleanliness",  0.55)
