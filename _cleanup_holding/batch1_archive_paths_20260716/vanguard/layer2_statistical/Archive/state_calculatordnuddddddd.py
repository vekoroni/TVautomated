...
class StateVectorCalculator:
    ...
    def _generate_state_hash(self, ticker: str, *states) -> str:
        ...
        Hash components (7 dimensions):
            vol_regime | trend_direction | trend_maturity | structure_quality
            | catalyst_proximity | adx_bucket | atr_pct_bucket

        NOTE: wyckoff_phase_bucket is NOT included here because state_hash
        must match the actuarial database columns. Add it once the database
        is rebuilt with the wyckoff_bucket column included.

        Reducing 123k matches → target 2k–15k per state bucket.
        """
        # Core 5 (original)
        ...
        signature_parts = [
            ...
            adx_bucket,               # adx_bucket       ← NEW
            atr_bucket,               # atr_pct_bucket   ← NEW
        ]
