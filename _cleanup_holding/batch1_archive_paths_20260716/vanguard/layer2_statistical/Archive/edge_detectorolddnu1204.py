...
class EdgeDetector:
    ...
    def detect_edge(self,
        ...
        # Fail-closed principle: if intraday_rows=0, Layer 1 auction
        # calculations are based on HTF data only — not real intraday
        # auction intelligence. We do NOT veto, but we must not let
        # a missing input silently masquerade as a valid calculation.
        # Downgrade auction_state to SEARCHING so Gate 1 doesn't
        # award false confidence, and flag it in the rationale.
        intraday_rows = getattr(state, 'intraday_rows', None)
        no_intraday = (intraday_rows is not None and intraday_rows == 0)
        if no_intraday:
            # Shadow the auction_state for gate purposes only — do not mutate the object
        ...
        net_ev = ev['ev_final']
        # FIX 2: Use P(return>0) directional win rate — NOT prob_up_10pct_20d (target hit rate).
        # REGIME_EV_FLOORS win_rate_floor values (0.38, 0.42, 0.48) were calibrated against
        # P(return>0). Using target hit rate (~68%) makes this floor trivially passable
        # and produces inflated STRONG verdicts on sub-50% directional win rate setups.
        win_rate = outcomes.win_rate  # P(return > 0) — directional win rate
    ...
    def _check_options_viability(self, state: StateVector) -> Dict:
        ...
        # ── LIQUIDITY HARD VETO (Fix 4 — enforce declared minimum) ─────────
        # LIQUIDITY_MIN_CONDITION = "LOW" means we veto if liquidity IS LOW.
        # Without this check the constant is declared but never enforced.
    ...
    def _calculate_confidence(self,
        ...
        """
        # Fix 6: When intraday data is absent, auction.confidence is based on
        # HTF data only — not real intraday tape. Apply a penalty to prevent
        # inflated confidence from flowing through to position sizing.
        intraday_rows = getattr(state, 'intraday_rows', None)
        no_intraday = (intraday_rows is not None and intraday_rows == 0)
    ...
    def _has_edge(self,
                  ...
                  intraday_warning: str = None) -> EdgeAssessment:
