...
class StateVectorCalculator:
    ...
    def calculate_state(self,
        ...
        def _phase_v2_calc(atr, adx, bb, rsi):
            if (atr < 50 and adx < 25 and bb < 60):
                return "EARLY_TRANSITION"
            elif (atr >= 50 and adx >= 25 and rsi < 70 and bb < 95):
                return "CONTINUATION"
            else:
                return "EXHAUSTION"
        ...
        return StateVector(
            ...
            adx_bucket=adx_bucket,
            intraday_rows=getattr(vanguard_input, 'intraday_rows', 0),
            # DEFECT 3 FIX: wyckoff_phase_bucket wired from compute at L91
            # Closes the gap: hash uses this dimension, query filter must receive it
            wyckoff_phase_bucket=wyckoff_bucket,
