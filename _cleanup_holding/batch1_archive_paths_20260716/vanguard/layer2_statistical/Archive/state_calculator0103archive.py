...
class StateVectorCalculator:
    ...
    def _calculate_volatility_regime(self, tech: 'TechnicalData') -> Dict:
        ...
        # IV percentile — use options layer ivp_252d if available, else options term structure,
        # else fall back to 50.0 (neutral). Hard-coding 50 permanently disables Gate 4
        # IV spike check and distorts vol_regime classification.
        iv_pct = 50.0  # default
        if options is not None:
            # Prefer 252-day IVP from options intelligence layer (most accurate)
            if hasattr(options, 'ivp_252d') and options.ivp_252d is not None:
                iv_pct = float(options.ivp_252d) * 100.0 if options.ivp_252d <= 1.0 else float(options.ivp_252d)
            elif hasattr(options, 'iv_percentile') and options.iv_percentile is not None:
                iv_pct = float(options.iv_percentile)
            elif hasattr(options, 'iv_rank') and options.iv_rank is not None:
                iv_pct = float(options.iv_rank)  # iv_rank is 0-100 percentile proxy
