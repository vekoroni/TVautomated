...
class ActuarialQueryEngine:
    ...
    def _find_similar_states(self, state: StateVector) -> pd.DataFrame:
        ...
        Stage 1 â€” Full 7-dimension match (preferred):
        ...
        Stage 2 â€” Core 4-dimension match (fallback when bucket columns
          don't exist in DB or no Stage 1 matches found):
          vol_regime + trend_direction + trend_maturity + structure_quality
          This is the pre-bucket behaviour â€” used as floor.

        The bucket columns (atr_pct_bucket, adx_bucket) were added after
        the initial DB build. Tickers whose bucket combination doesn't
        exist in the DB would silently return 0 matches without this fallback.
        """
        df = self.df.copy()

        # â”€â”€ STAGE 1: Full discriminated match â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        s1 = df.copy()
        s1 = s1[s1['vol_regime'] == state.vol_regime]
        s1 = s1[s1['trend_direction'] == state.trend_direction]

        # trend_maturity filter — v2.2.0: SIDEWAYS_BUILDING / SIDEWAYS_RANGING
        # replaced N/A. Pre-rebuild DB rows still carry N/A, so we include N/A
        # in the allowed set to avoid returning 0 matches during transition.
        # After rebuild completes, N/A rows disappear and this is a no-op.
        ...
        # NEW: wyckoff_phase_bucket — separates ACCUMULATION (A/B/C) from
        # MARKUP (D/E) from DISTRIBUTION. Most impactful single dimension.
        # Only filters when DB has the column (post-rebuild) and bucket is known.
        ...
        # NEW: macro_regime — RISK_ON and RISK_OFF Phase D setups have
        # materially different win rates and must not pool together.
        # Allow TRANSITIONAL rows to match any regime (bridge rows).
        ...
        # â”€â”€ STAGE 2: Core 4-dimension fallback (bucket columns missing/empty) â”€
        # This happens when atr_pct_bucket or adx_bucket in the DB don't
        # contain values matching this ticker's computed buckets.
        # Degrades gracefully to pre-bucket discrimination.
        s2 = df.copy()
