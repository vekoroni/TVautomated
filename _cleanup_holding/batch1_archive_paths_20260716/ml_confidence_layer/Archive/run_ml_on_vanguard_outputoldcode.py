...
def map_row(row: pd.Series, macro_regime: str) -> VanguardSignal:
    ...
    if vol_regime in ("LOW", "COMPRESSED"):
        compression = "COMPRESSED"
    elif vol_regime in ("HIGH", "EXPANDED"):
        compression = "EXPANDED"
    else:
        compression = "NORMAL"

    # trend_direction -> wyckoff_phase proxy
    trend = str(get("layer2__trend_direction", "unknown")).lower()
    ...
    # layer1__control -> control_state
    control_raw = str(get("layer1__control", "NEUTRAL")).upper()
    if "BULL" in control_raw or "BUY" in control_raw:
        control = "BUYERS"
    elif "BEAR" in control_raw or "SELL" in control_raw:
        control = "SELLERS"
    else:
        control = "NEUTRAL"
    ...
    flow_proxy = min(max(50.0 + sharpe * 10, 0), 100)
    ...
    return VanguardSignal(
        ...
        ev                 = float(get("layer2__expected_value_20d", 1.0)),
