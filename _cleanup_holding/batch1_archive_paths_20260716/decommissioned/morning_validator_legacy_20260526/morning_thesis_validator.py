...
# ── Phantom 1: Interpreter Sidecar Integration ───────────────────────────────
# These functions are additive only. If the interpreter sidecar is absent,
# the morning validator MUST function normally using pipeline data alone.

def load_interpreter_sidecar(ticker: str, run_id: str) -> Optional[Dict]:
    """
    Load the interpreter sidecar JSON for a given ticker and run_id.
    Returns None if not found — callers must handle None gracefully.
    Sidecar is strictly additive; never blocks or modifies verdict logic.
    """
    sidecar_path = RUNS_DIR / run_id / "interpreter" / f"{run_id}_{ticker.upper()}_interpreter.json"
...
def apply_interpreter_overlay(candidate: Dict, sidecar: Optional[Dict]) -> Dict:
    """
    Apply interpreter sidecar display fields to a morning candidate row.
    These are DISPLAY-ONLY overlays — they never modify pipeline verdict logic.
    All new fields are prefixed interpreter_ to prevent confusion with pipeline fields.

    EIL catalyst override is GO→GO_LIMIT only (one level downgrade). Never BLOCKED.
    Capital denial remains the trader's decision.

    If sidecar is None, returns candidate unchanged.
    """
    if not sidecar:
        return candidate

    # Add display-only fields
    candidate["interpreter_crowd_stage"]     = sidecar.get("crowd_stage") or ""
    candidate["interpreter_p_trigger"]       = sidecar.get("p_trigger_hit") or ""
    candidate["interpreter_edge_quality"]    = sidecar.get("edge_quality") or ""
    candidate["interpreter_garch_status"]    = sidecar.get("garch_status") or ""
    candidate["interpreter_entry_quality"]   = sidecar.get("entry_quality") or ""
    candidate["interpreter_junior_ok"]       = str(sidecar.get("junior_sections_present", ""))
    candidate["interpreter_ts"]              = sidecar.get("ts") or ""

    # Catalyst override: GO → GO_LIMIT only, never BLOCKED
    cat_override = sidecar.get("catalyst_override")
    if cat_override == "GO_LIMIT":
        current_permission = str(candidate.get("execution_permission", "")).strip().upper()
        if current_permission == "GO":
            candidate["execution_permission"] = "GO_LIMIT"
            candidate["interpreter_cat_override"] = "GO→GO_LIMIT (catalyst conflict)"

    return candidate


def enrich_candidates_with_sidecars(candidates: List[Dict], run_id: str) -> Tuple[List[Dict], int]:
    """
    Batch-apply interpreter sidecars to a list of morning candidates.
    Returns (enriched_candidates, sidecar_count).
    Gracefully skips any ticker where sidecar is not found.
    """
    enriched = []
    loaded = 0
    for c in candidates:
        ticker  = str(c.get("ticker", "")).upper()
        sidecar = load_interpreter_sidecar(ticker, run_id) if ticker else None
        if sidecar:
            loaded += 1
        enriched.append(apply_interpreter_overlay(c, sidecar))
