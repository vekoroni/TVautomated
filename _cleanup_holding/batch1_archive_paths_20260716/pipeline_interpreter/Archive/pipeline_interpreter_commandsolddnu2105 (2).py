...
from pipeline_interpreter_engine import (
    ...
    EXECUTION_PERMISSION, MODEL_TRIAGE, MODEL_DEEP_DIVE, LIVE_PRICES,
)
...
â•‘  /intraday TICKER [IMG...]    15m/30m chart + live price analysis â•‘
â•‘  /price TICKER PRICE [chg%]  Register live price for analysis   â•‘
...
def cmd_triage(file_path:str=None):
    ...
    if response:
        ...
        response = call_api(prompt, model=MODEL_TRIAGE)  # Sonnet 4.6 — fast triage
...
def cmd_price(args: str = ""):
    ...
    # No args — show all registered prices
    if not args:
        if not LIVE_PRICES:
    ...
    if len(tokens) == 2 and tokens[1].lower() == "clear":
        if ticker in LIVE_PRICES:
            ...
            print(f"  ✅ Live price for {ticker} cleared.")
        else:
    ...
    try:
        ...
        print(f"  ⚠  Invalid price: {tokens[1]}")
        return
    ...
    print(f"  ✅ Live price registered: {ticker} = ${price}{chg}{vwap}  [{entry['timestamp']}]")
    print(f"  Will be injected into /ticker {ticker} and /intraday {ticker} automatically.")


def cmd_intraday(args: str = ""):
    """
    Intraday chart analysis — 15m/30m charts + live price context.

    Usage:
      /intraday WFC                           (no charts — price-only update)
      /intraday WFC 15m_chart.png             (single chart)
      /intraday WFC chart1.png chart2.png     (multiple charts — e.g. 15m + 30m)
    ...
    for tok in tokens[1:]:
        ...
        if p.exists():
            ...
            print(f"  ⚠  Image not found: {tok} — skipping")
    ...
    if not row:
        print(f"  ⚠  {ticker} not found in session CSV. Analysis will use live price only.")
        row = {"ticker": ticker, "note": "Not found in pipeline CSV — live price analysis only"}
    ...
    if not live_price_block:
        print(f"  ⚠  No live price registered for {ticker}.")
    ...
    print(f"  Charts: {len(all_images)} image(s) — timeframes: {', '.join(chart_timeframes)}")
    if live_price_block:
