...
def cmd_brief(text: str = ""):
    """
    Paste a newsroom brief directly into the session.

    Usage:
        /brief                   (prompts for multi-line paste — paste then press Enter twice)
        /brief TEXT              (inline for short text)

    The brief is saved to MA_Inputs\\news_terminal\\newsroom_brief_latest.txt
    and is automatically read by /triage and /ticker without any further action.

    Format: paste the brief exactly as it arrives from the news terminal —
    the reader handles Alert Level, ANIS, FIPS, numbered sections, and plain text.
    """
    if text.strip():
        # Text provided inline on the /brief command line
        brief_text = text.strip()
    else:
        # Interactive multi-line paste
        print("  Paste the newsroom brief below.")
    ...
    if not brief_text:
        print("  ⚠  No text received — brief not saved.")
        return

    saved_path = save_pasted_brief(brief_text)
    lines_count = brief_text.count("\n") + 1
    print(f"  ✅ Brief saved ({lines_count} lines)")
    print(f"  Will be read automatically on next /triage or /ticker run.")
    print(f"  Run /triage now to apply it to today\'s candidates.")
...
def route_command(raw:str):
    ...
    if   cmd=="/menu":    print(MENU)
        ...
        # Accept text on the same line or drop into interactive paste
