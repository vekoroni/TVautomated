...
from news_macro_readers import save_pasted_brief, show_status as sync_show_status, MA_PIPELINE_OUT
...
def cmd_triage(file_path:str=None):
    ...
    if file_path:
        ...
        # Auto-scan MA_Inputs — select correct file based on session mode and run_id
        import json as _json
        _active_run_id  = None
        _session_mode   = "EOD"
        _session_marker = MA_INPUTS / "session_state.json"
        if _session_marker.exists():
            try:
                _sm = _json.loads(_session_marker.read_text(encoding="utf-8"))
                _active_run_id = _sm.get("run_id", "").strip()
                _session_mode  = _sm.get("session_mode", "eod").upper()
            except Exception:
                pass
        ...
        if _active_run_id and found:
            if _session_mode == "MORNING":
                # Morning: prefer morning_validated_trades for this run_id
                _mv = found.get("morning_validated")
                if _mv and _active_run_id in str(_mv):
                    best = _mv
                    print(f"  [MODE: MORNING] morning_validated_trades run {_active_run_id}")
                if not best:
                    _mc = found.get("morning_candidates")
                    if _mc and _active_run_id in str(_mc):
                        best = _mc
                        print(f"  [MODE: MORNING] falling back to morning_candidates run {_active_run_id}")
            else:
                # EOD: prefer morning_candidates for this run_id
                _mc = found.get("morning_candidates")
                if _mc and _active_run_id in str(_mc):
                    best = _mc
                    print(f"  [MODE: EOD] morning_candidates run {_active_run_id}")
                if not best:
                    _mv = found.get("morning_validated")
                    if _mv and _active_run_id in str(_mv):
                        best = _mv
                        print(f"  [MODE: EOD] falling back to morning_validated run {_active_run_id}")

        # Fallback: any file with the active run_id
        if not best and _active_run_id and found:
            for _k, _p in found.items():
                if _active_run_id in str(_p):
                    best = _p
                    print(f"  [FALLBACK] {Path(_p).name} (run_id match)")
                    break

        # Last resort: newest morning_candidates regardless of run
        if not best and found:
            best = (
                found.get("morning_candidates")
                or found.get("morning_validated")
                or found.get("execution")
                or found.get("eil")
                or list(found.values())[0]
            )
            print(f"  [FALLBACK] No run_id match — using newest available file")
...
def cmd_brief(text: str = ""):
    """Paste newsroom brief. Type /brief then paste, type END to finish."""
    if text.strip():
        print(f"  ⚠  Only captured one line. Type /brief alone, paste brief, then END")
        lines = [text.strip()]
    else:
        lines = []
    ...
    brief_text = "\n".join(lines).strip()
    if not brief_text:
        print("  ⚠  No text received. Type /brief, paste, then END.")
        return
    saved_path = save_pasted_brief(brief_text)
    lines_count = brief_text.count("\n") + 1
    print(f"\n  ✅ Brief saved ({lines_count} lines)")
    print(f"  Newsroom context included automatically in /triage and /ticker.")
