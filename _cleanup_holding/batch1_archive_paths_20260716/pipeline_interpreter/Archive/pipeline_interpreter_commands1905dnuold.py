...
def cmd_brief(text: str = ""):
    ...
    CORRECT USAGE:
        PI> /brief
        Paste the newsroom brief. Type END on a new line when done.

    Do NOT type the brief text on the same line as /brief.
    That only captures the first line.
    """
    if text.strip():
        print(f"  ⚠  Only captured one line: '{text[:60]}'")
        print(f"  Type /brief on its own line, paste the full brief, then type END\n")
        lines = [text.strip()]
    else:
        lines = []
    ...
    if not brief_text:
        ...
        print("  Type /brief, paste the brief, then type END on a new line.")
        return
