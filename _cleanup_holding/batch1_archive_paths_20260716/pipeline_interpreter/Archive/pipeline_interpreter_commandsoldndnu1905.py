...
def cmd_triage(file_path:str=None):
    ...
    if file_path:
        ...
        _active_run_id = None
        _session_mode  = "EOD"
        _sm_path = MA_INPUTS / "session_state.json"
        if _sm_path.exists():
            try:
                _sm = _json.loads(_sm_path.read_text(encoding="utf-8"))
        ...
        best  = None  # always initialised here
        ...
        if not best and found:
            ...
            print(f"  [FALLBACK] No run_id match — using newest available")
    ...
    if file_path:
        ...
        _fallback = (
            found.get("morning_candidates") or found.get("morning_validated")
            or found.get("execution") or found.get("eil")
            or list(found.values())[0]
        )
        SESSION["last_pipeline_csv"] = str(_fallback)
...
def cmd_brief(text: str = ""):
    """Paste newsroom brief. Type /brief alone, paste, then END."""
    if text.strip():
