...
# SPRINT 3: Gate 1 expanded from Phase E only → Phase D + E.
# Phase D (markup confirmation) produces valid 0DTE setups when Crabel
# compression is confirmed. POST_EARNINGS_MOMENTUM bypasses phase gate entirely.
GATE_PHASE = ['D', 'E']               # Gate 1: Phase D (with crabel) or Phase E
GATE_CRABEL_REQUIRED_FOR_D = True     # Gate 1b: Phase D requires crabel compression
GATE_EARNINGS_BYPASS_FLAG  = 'POST_EARNINGS_MOMENTUM'  # bypasses Gate 1 entirely
...
def gate_1_phase(df: pd.DataFrame, logger: logging.Logger) -> pd.DataFrame:
    """
    Gate 1: Phase D (with Crabel compression) or Phase E.
    Phase E = mark-down/mark-up confirmed — fastest phase, ideal for 0DTE.
    Phase D = markup confirmation — valid for 0DTE only when Crabel compression
              is also confirmed (crabel_state = CRABEL_READY or COILING).

    SPRINT 3: POST_EARNINGS_MOMENTUM flag bypasses phase gate entirely.
    Post-earnings momentum stocks are valid 0DTE candidates regardless of
    Wyckoff phase — the catalyst drives the move, not the structural phase.
    """
    if 'phase' not in df.columns:
        logger.warning("Gate 1: 'phase' column not found — skipping phase filter")
        return df

    # POST_EARNINGS_MOMENTUM bypass — phase gate skipped for event-driven setups
    earnings_bypass = pd.Series(False, index=df.index)
    if 'signal_type' in df.columns:
        earnings_bypass = df['signal_type'].str.upper().str.contains(
            GATE_EARNINGS_BYPASS_FLAG, na=False
        )
    if 'catalyst_flag' in df.columns:
        earnings_bypass = earnings_bypass | df['catalyst_flag'].str.upper().str.contains(
            GATE_EARNINGS_BYPASS_FLAG, na=False
        )

    # Phase E: always passes
    phase_e = df['phase'].isin(['E'])

    # Phase D: passes only with Crabel compression
    if GATE_CRABEL_REQUIRED_FOR_D and 'crabel_state' in df.columns:
        phase_d = (
            df['phase'].isin(['D']) &
            df['crabel_state'].isin(['CRABEL_READY', 'COILING'])
        )
    else:
        phase_d = df['phase'].isin(['D'])

    passed = df[earnings_bypass | phase_e | phase_d].copy()
    logger.info(
        f"Gate 1 (Phase D/E + earnings bypass): {len(df)} → {len(passed)} signals "
        f"[E:{phase_e.sum()} D:{phase_d.sum()} bypass:{earnings_bypass.sum()}]"
    )
    return passed
