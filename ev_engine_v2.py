...
PATCH-05 (Sprint 3 / v2.2.0): V2 actuarial signal intelligence integration
  - EVInputs extended: signal_type, momentum_tier, fwd_momentum_conf
  - ev_inputs_from_row(): derives predictability_score from momentum_tier,
    bmps from momentum_bucket, calibration boost from fwd_momentum_conf
    when not supplied by upstream (Superbrain/flow). Defaults unchanged.
  - _regime_mult(): signal-tier boost (+0.02–+0.08) for CONTINUATION signals
    on top of existing regime × drift multiplier. TRANSITION: no change.
  - All 4-layer architecture, hard gates, contract math: UNCHANGED.
"""
...
class EVInputs:
    ...
    # ── V2 SIGNAL FIELDS (Sprint 3) — all default to neutral values ──────────
    # Populated by ev_inputs_from_row() when state_row contains V2 data.
    # Used by _regime_mult() for signal-aware multiplier boost.
    signal_type:str="NO_EDGE"       # CONTINUATION | TRANSITION | NO_EDGE
    momentum_tier:str="TIER_4_FLAT" # TIER_1_EXPLOSIVE | TIER_2_SUSTAINING | TIER_3_BUILDING | TIER_4_FLAT
    fwd_momentum_conf:float=0.0     # 0.0–1.0, fraction of DB rows with bucket rank increase
...
def ev_inputs_from_row(row:dict) -> EVInputs:
    ...
    # Derive predictability_score and bmps from actuarial V2 signal fields
    # when not already supplied by upstream (e.g. Superbrain, options flow).
    # These default to 50.0 in EVInputs — far too conservative for CONTINUATION.
    #
    # predictability_score: how well the actuarial phase predicts outcome direction
    #   TIER_1_EXPLOSIVE / TIER_2_SUSTAINING → 72%  (HIGH/EXTREME → EXTREME/HIGH)
    #   TIER_3_BUILDING                      → 65%  (MID → HIGH/EXTREME building)
    #   TIER_4_FLAT                          → 60%  (MID → MID flat continuation)
    #   default                              → composite or 50
    #
    # bmps (Base Momentum Probability Score): momentum quality at entry
    #   EXTREME bucket → 78%
    #   HIGH bucket    → 70%
    #   MID bucket     → 58%
    #   LOW bucket     → 45%
    #   default        → 50
    #
    # fwd_momentum_conf boosts calibration_confidence by up to +10%
    # (fraction of DB rows with bucket rank increase — genuine acceleration).

    _signal_type   = s('signal_type', 'NO_EDGE').upper()
    _mom_tier      = s('momentum_tier', 'TIER_4_FLAT').upper()
    _mom_bucket    = s('momentum_bucket', 'LOW').upper()
    _fmc           = _safe(row.get('fwd_momentum_conf'), 0.0)

    _ps_supplied   = row.get('predictability_score') or row.get('predictability')
    _bmps_supplied = row.get('bmps')

    # Derive predictability_score from momentum tier if not supplied upstream
    if not _ps_supplied and _signal_type == 'CONTINUATION':
        _derived_ps = {
            'TIER_1_EXPLOSIVE':   72.0,
            'TIER_2_SUSTAINING':  70.0,
            'TIER_3_BUILDING':    65.0,
            'TIER_4_FLAT':        60.0,
        }.get(_mom_tier, 60.0)
    elif not _ps_supplied and _signal_type == 'TRANSITION':
        _derived_ps = {
            'TIER_1_ACCELERATING': 58.0,
            'TIER_2_BUILDING':     54.0,
        }.get(_mom_tier, 52.0)
    else:
        _derived_ps = None  # use existing fallback chain

    # Derive bmps from momentum_bucket if not supplied upstream
    if not _bmps_supplied:
        _derived_bmps = {
            'EXTREME': 78.0,
            'HIGH':    70.0,
            'MID':     58.0,
            'LOW':     45.0,
        }.get(_mom_bucket, 50.0)
    else:
        _derived_bmps = None

    # Calibration confidence boost from forward momentum evidence
    _cc_supplied = row.get('calibration_confidence')
    _cc_fmc_boost = _fmc * 10.0  # +10% max boost when 100% DB rows accelerating

    def _predictability():
        if _derived_ps is not None: return _derived_ps
        return _safe(row.get('predictability_score'),
               _safe(row.get('predictability'),
               _safe(row.get('composite'), 50.0)))

    def _bmps_val():
        if _derived_bmps is not None: return _derived_bmps
        return _safe(row.get('bmps'), 50.0)

    def _calibration():
        base = _safe(row.get('calibration_confidence'),
               _safe(row.get('composite'), 50.0))
        return _clamp(base + _cc_fmc_boost, 0.0, 100.0)
    ...
    return EVInputs(
        ...
        predictability_score=_predictability(),
        bmps=_bmps_val(),
        calibration_confidence=_calibration(),
        ...
        # V2 signal fields — populated from state_row V2 data
        signal_type=_signal_type,
        momentum_tier=_mom_tier,
        fwd_momentum_conf=_fmc,
    )
...
class EVEngineV2:
    ...
    def _regime_mult(self,x):
        ...
        base = b*d
        # ── V2 Signal-tier boost (Sprint 3) ──────────────────────────────────
        # Applies on top of regime × drift base. Rationale: actuarial Stage 0
        # CONTINUATION tiers represent a systematically higher-quality state
        # than regime alone can express. RISK_ON + TIER_1_EXPLOSIVE is a
        # fundamentally different edge than RISK_ON + EXHAUSTION.
        # Caps at 1.20 (existing max) — cannot exceed existing ceiling.
        # TRANSITION: no boost — probe signals do not warrant EV inflation.
        _tier_boost = {
            'TIER_1_EXPLOSIVE':   0.08,  # HIGH/EXTREME → EXTREME: max confidence
            'TIER_2_SUSTAINING':  0.08,  # HIGH → HIGH: strong steady edge
            'TIER_3_BUILDING':    0.04,  # MID → HIGH/EXTREME: building
            'TIER_4_FLAT':        0.02,  # MID → MID: flat continuation
        }.get(getattr(x,'momentum_tier','TIER_4_FLAT'), 0.0)
        # Only apply boost for CONTINUATION signals
        if getattr(x,'signal_type','NO_EDGE') == 'CONTINUATION':
            base = base + _tier_boost
        return _clamp(base,0.50,1.20)
