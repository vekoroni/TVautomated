...
| 3 — expression marks (§5.3) | Done, uncommitted. `c12-expression-v1.0.0`: entry at recorded ask, exit at end-of-day bid on the underlying's resolution / session-20 / last usable session (expiry − `outcome.contract_exit_buffer`); no substitution. New key `outcome.contract_multiplier` (100, PROVISIONAL). Of 12,546 contracts: 2,715 MARKED, 2,486 ENTRY_NOT_VALUED (641 have an evidence-session chain ask, stored but unused pending ACK), 1,866 MARK_UNAVAILABLE (1,421 in Jul–Aug, weekly chains), 1,409 CONTRACT_INVALID, 4,070 pending. Recorded asks agree with the chain (median ratio 1.00; 70% within 5%). |

First expression result (headline, INSUFFICIENT_SESSIONS): mean return on premium −45.3% (−50.4 to −38.0), win rate 15%; with the underlying reaching target first, options returned median +0.9% / mean +17.8% against +5.6% on the underlying; on stop-first −69.4%. Caveat: the exit is the end-of-day bid on the touch session, not a fill at the level.
