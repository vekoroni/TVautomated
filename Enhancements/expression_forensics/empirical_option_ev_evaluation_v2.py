"""Old vs enhanced empirical_option_ev on the SAME realised option marks (read-only).
...
OUT = Path(__file__).resolve().parent / "empirical_option_ev_evaluation_v2.json"
...
def main():
    ...
    for path in sorted(glob.glob(str(MAIN / "data/output/runs/*/options/options_intelligence_*.csv"))):
        ...
        for r in frame.to_dict("records"):
            ...
            o, n = old.compute_empirical_option_ev(dict(cand)), new.compute_empirical_option_ev(dict(cand))
    ...
    out = {"joined": len(df), "forecast_source": df.forecast_source.value_counts().to_dict(),
           ...
           "OLD": summarise(both, "old_expected_r"), "NEW": summarise(both, "new_expected_r")}
