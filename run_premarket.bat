...
cd /d "%REPO_ROOT%"
echo ============================================================
echo AVSHUNTER - PRODUCTION MORNING WORKFLOW
echo Entrypoint: intelligent_orchestrator.py --morning
...
"%PYTHON_EXE%" "%REPO_ROOT%\intelligent_orchestrator.py" --morning %*
set "RESULT=%ERRORLEVEL%"

if not "%RESULT%"=="0" (
  echo [FAILED] Morning workflow returned exit code %RESULT%.
) else (
  echo [COMPLETE] Morning workflow finished successfully.
)
exit /b %RESULT%
